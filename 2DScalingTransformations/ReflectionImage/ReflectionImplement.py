...
def f_getExtents(T, rowMax, colMax):
...
def f_transform(T, I_gray):
    ...
    min_row, min_col, max_row, max_col, H, W = f_getExtents(T, I_gray.shape[0], I_gray.shape[1])
...
# Hàm reflect image bằng opencv | axis = 0 -> x, axis = 1 -> y
def reflect_image(I_gray, reflect_axis):
    return cv2.flip(I_gray, reflect_axis)
...
if __name__ == '__main__':
    ...
    # Matrix reflect (phản chiếu) by x axis (col)
    Tx = np.array([[-1, 0], [0, 1]])
    # Matrix reflect (phản chiếu) by y axis (row)
    Ty = np.array([[1, 0], [0, -1]])
    # Sử dụng hàm biến đổi tự implement
    # I2 = f_transform(Tx, I_gray)

    # Sử dụng opencv
    # I2 = reflect_image(I_gray, 1)

    # Shear Image
    k = 0.2

    # Matrix nghiêng ảnh theo trục x
    Tx = np.array([[1, 0], [k, 1]])
    # Matrix nghiêng ảnh theo trục y
    Ty = np.array([[1, k], [0, 1]])
    I2 = f_transform(Ty, I_gray)
